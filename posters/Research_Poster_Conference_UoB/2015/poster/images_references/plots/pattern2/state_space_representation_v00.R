...
# 
#  Miguel Angel Perez-Xochicale <perez.xochicale AT gmail.com>
...
raw_emy <- read.csv(paste(emy,"",sep=""),header = FALSE, sep='')
w_emy <- zeromean_unitvariance(raw_emy)

embeddedmatrix_emy <- Takens_Theorem(w_emy,10,1,1)
pcamatrix_emy <- PCA(embeddedmatrix_emy,0)

filenameimage <- paste("ssr_ep2",".png",sep="") 
png(filenameimage, width=2600, height=2600, units="px", res=200,bg = "transparent")
xyplot(-pcamatrix_emy[[3]][1,] ~ pcamatrix_emy[[3]][2,],
       type = c("l","g"), lwd=10, col="black",
       xlab=list(label="PC2", cex=4),
       ylab=list(label="-PC1", cex=4),
       grid = FALSE,
       #scales=list(font=1, cex=3, y=list(at=seq(-8,8,2),limits=c(-7.5,7.5)),x=list(at=seq(-4,4,2),limits=c(-5.5,5.5)) ),
...
raw_imy <- read.csv(paste(imy,"",sep=""),header = FALSE, sep='')
w_imy <- zeromean_unitvariance(raw_imy)

embeddedmatrix_imy <- Takens_Theorem(w_imy,10,1,1)
pcamatrix_imy <- PCA(embeddedmatrix_imy,0)

filenameimage <- paste("ssr_ip2",".png",sep="") 
png(filenameimage, width=2600, height=2600, units="px", res=200,bg = "transparent")
xyplot(pcamatrix_imy[[3]][1,] ~ pcamatrix_imy[[3]][2,],
...
raw_nmy <- read.csv(paste(nmy,"",sep=""),header = FALSE, sep='')
w_nmy <- zeromean_unitvariance(raw_nmy)

embeddedmatrix_nmy <- Takens_Theorem(w_nmy,10,1,1)
pcamatrix_nmy <- PCA(embeddedmatrix_nmy,0)

filenameimage <- paste("ssr_np2",".png",sep="") 
png(filenameimage, width=2600, height=2600, units="px", res=200,bg = "transparent")
xyplot(pcamatrix_nmy[[3]][1,] ~ -pcamatrix_nmy[[3]][2,],
...
Plot_Cumulative_Energy(pcamatrix_emy[[6]],pcamatrix_imy[[6]],pcamatrix_nmy[[6]],10)