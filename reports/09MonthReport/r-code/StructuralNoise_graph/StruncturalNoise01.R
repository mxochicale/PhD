...
plot <- xyplot(time.serie ~ ts,
  ...
  xlab=list(label="", cex=4),
  ylab=list(label="", cex=4),
  scales=list(font=1, cex=3,
                   y=list(at=seq(-2,2,1),limits=c(-2.5,2.5))
...
time     <- 10                    # measuring time interval (seconds)
ts       <- seq(0,time-1/acq.freq, 1/acq.freq) # vector of sampling time-points (s) 


dc.component <- 0
frequency <- 1      # frequency of signal component (Hz)
delay <- 0          # delay of signal component (radians)

wl <- 250         # window lenght
freq.sd <- 2   # strunctural noise -- frequency standard deviation 
ad.sd <- 4     # additive noise standard deviation
...
#plot.timeserie(tts, ts)

x <- as.matrix(as.numeric(tts))
xw <- zeromean_unitvariance(  t(x) )


png(filename="sngraph.png", height=500, width=1000,bg="white")
plot.timeserie(xw, ts)
