...
\IEEEPARstart{H}{uman} Activity Recognition (HAR) using body-worn sensors has increased 
...
% in which repetition of 5 clasess of activities 
% (1 specified execution and 4 common mistakes of the exercise) were classified using statistical 
% features (mean, variance, standard deviation, etc.).
...
% Velloso \emph{et al.} \cite{Velloso2013b} used a model based on timestamped characteristic points 
% in the time series which are compared using $k-$means clustering.
...
% since ,as pointed by Sama \emph{et al.} \cite{Sama2013}, $\tau_{min}$ value depends on the application 
% at hand.
% and therfore 
% the quality of the phase space portrait is 


%%%%%%%%%%%%%%%%%%%%
% EXTEND THIS PARAGRAPH FOR THE CURRENT ADVANCES IN MI AND CAO' ALGORITHM
% It is important to point out that the mutual infomation \cite{Thomas2014}
% as well the noise ..
% Thus, there is still research to be done to find the minimal 
