...
%   \item For classification purposes, are the time-delay embedding and PCA methods
%  the better tools for feature extraction compared to other state-of-the-art 
%  feature extraction methods \cite{bulling2014} ?
...
%  \item How will the uncertainty pervading the sensors outcomes 
%  (stochastic) be combined with (subjective) uncertainty of dancer's movement? 
 
%  \item How will the evaluation process be carried out?
\end{enumerate}
...
% From orientation angles of 
% three IMUs, five indicators of the rowing technique 
% (stroke rate, stroke lenght, ratio recovery/drive, ratio feathered/squared)
% are used 
...
i) noise in sensors,
% : ? needs to be removed? *hammerla*
ii) properties of activities and 
% : define diferences in activities,
% and need to make sure actions are carefully selected and
iii) properties of people.
% : this variability is what you want to measure and analyse
...
% 
% \section{Still Working On:}
% \begin{itemize}
%  \item hammerla method to takenspca
%  \item results with nondancers data
% \end{itemize}
...
% we are planning to 
% 
% Similarly, my plan is to classify the collecting data according to the better feature
% representation as well as the evaluation of the current approach for 
% using the variation of activities as a metric for skills assessment in sawing and tennis activities.
...
% \textbf{ADD WIDER FRAMEWORK OF THE PHD}
